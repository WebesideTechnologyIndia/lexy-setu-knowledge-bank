...
from .forms import (
    CategoryForm, SubCategoryForm, ActForm, ChapterForm, 
    SectionForm, RuleForm, FormForm, NotificationForm
)
...
def home_view(request):
    ...
    try:
        ...
        categories = Category.objects.filter(
            ...
            'subcategories__chapters__acts'  # This is CORRECT path
        ).order_by('order', 'name')
...
def category_detail_view(request, slug):
    ...
    try:
        ...
        for act in all_acts:
            print(f"  - {act.name} (Chapter: {act.chapter.name}, SubCat: {act.chapter.subcategory.name})")
...
def act_detail_view(request, category_slug, subcategory_slug, chapter_slug, slug):
    ...
    try:
        ...
        notifications = act.notifications.filter(is_active=True).order_by('-notification_date')
        
        # Get related acts in the same chapter
        related_acts = Act.objects.filter(
            chapter=chapter,
            is_active=True
        ).exclude(id=act.id).order_by('order', 'name')[:5]
        ...
        context = {
            ...
            'breadcrumbs': [
                ...
                {'name': 'Acts', 'url': '/acts/'},
                {'name': category.name, 'url': f'/acts/category/{category.slug}/'},
                {'name': subcategory.name, 'url': f'/acts/category/{category.slug}/subcategory/{subcategory.slug}/'},
                {'name': chapter.name, 'url': f'/acts/category/{category.slug}/subcategory/{subcategory.slug}/chapter/{chapter.slug}/'},
    ...
    try:
        section = get_object_or_404(
            Section.objects.select_related(
                'act__category',
                'act__chapter__subcategory__category',
                'parent_section'
            ).prefetch_related(
                'sub_sections',
                'rules',
        ...
        # Get sub-sections
        sub_sections = section.sub_sections.filter(is_active=True).order_by('order', 'section_number')
        ...
        context = {
            ...
            'breadcrumbs': [
                ...
                {'name': 'Acts', 'url': '/acts/'},
                {'name': category.name, 'url': f'/acts/category/{category.slug}/'},
                {'name': act.name, 'url': f'/acts/category/{category.slug}/act/{act.slug}/'},
...
def search_view(request):
    ...
    if query and len(query) >= 2:
        ...
        total_results = sum([
            ...
            len(rule_results), len(form_results)
        ])
...
def add_content_home(request):
    ...
    try:
        ...
        return render(request, 'acts/add_content/dashboard.html', {
            'categories_count': 0,
...
def get_chapters(request):
    ...
    data = [{'id': chapter.id, 'name': f"Chapter {chapter.chapter_number}: {chapter.name}"} for chapter in chapters]
...
def get_sections(request):
    ...
    data = [{'id': section.id, 'name': f"Section {section.section_number}: {section.name}"} for section in sections]
...
def add_chapter_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            chapter.save()
            messages.success(request, f'Chapter "{chapter.name}" has been created successfully!')
...
def get_navigation_data(request):
    ...
    try:
        ...
        for category in categories:
            ...
            # Add subcategories and their hierarchy
            for subcategory in category.subcategories.filter(is_active=True).order_by('order', 'name'):
                subcategory_data = {
                    'id': subcategory.id,
                    'name': subcategory.name,
                    'slug': subcategory.slug,
                ...
                for chapter in subcategory.chapters.filter(is_active=True).order_by('order', 'chapter_number'):
                    ...
                    for act in chapter.acts.filter(is_active=True).order_by('order', 'name'):
                        act_data = {
                            'id': act.id,
                            'name': act.name,
                            'slug': act.slug,
...
def rule_detail_view(request, category_slug, subcategory_slug, chapter_slug, act_slug, slug):
    ...
    try:
        ...
        context = {
            ...
            'breadcrumbs': [
                ...
                {'name': subcategory.name, 'url': f'/acts/category/{category.slug}/subcategory/{subcategory.slug}/'},
                {'name': chapter.name, 'url': f'/acts/category/{category.slug}/subcategory/{subcategory.slug}/chapter/{chapter.slug}/'},
                {'name': act.name, 'url': f'/acts/category/{category.slug}/subcategory/{subcategory.slug}/chapter/{chapter.slug}/act/{act.slug}/'},
...
def form_detail_view(request, category_slug, subcategory_slug, chapter_slug, act_slug, slug):
...
def notification_detail_view(request, category_slug, subcategory_slug, chapter_slug, act_slug, slug):
...
def act_detail_simple(request, slug):
    ...
    try:
        ...
        return redirect('acts:act_detail', 
...
def section_detail_view(request, category_slug, subcategory_slug, chapter_slug, act_slug, slug):
    """Display section details"""
    try:
        section = get_object_or_404(
            Section.objects.select_related(
    ...
    try:
        ...
        context = {
            ...
            'breadcrumbs': [
                {'name': 'Home', 'url': '/'},
                {'name': 'Acts', 'url': '/acts/'},
                {'name': act.chapter.subcategory.category.name, 'url': f'/acts/category/{act.chapter.subcategory.category.slug}/'},
                {'name': act.chapter.subcategory.name, 'url': f'/acts/category/{act.chapter.subcategory.category.slug}/subcategory/{act.chapter.subcategory.slug}/'},
                {'name': act.chapter.name, 'url': f'/acts/category/{act.chapter.subcategory.category.slug}/subcategory/{act.chapter.subcategory.slug}/chapter/{act.chapter.slug}/'},
                {'name': act.name, 'url': f'/acts/category/{act.chapter.subcategory.category.slug}/subcategory/{act.chapter.subcategory.slug}/chapter/{act.chapter.slug}/act/{act.slug}/'},
                {'name': 'Rules', 'url': None}
            ]
        }
        return render(request, 'acts/rules_list.html', context)
...
def notifications_list_view(request, category_slug, subcategory_slug, chapter_slug, act_slug):
    """List all notifications for an act"""
    try:
        act = get_object_or_404(
            Act.objects.select_related('chapter__subcategory__category'),
